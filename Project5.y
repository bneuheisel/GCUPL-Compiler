...
plus_minus
	: mul_div_mod  				{$$=$1;}
	| plus_minus '+' mul_div_mod  {if($1->array == 200 && $3->array==200){ //means that both sides are constants
								printf("constant folding\n");
								int a = $1->lexeme - '0';
								int b = $3->lexeme - '0';
								char cons[100];
								sprintf(cons, "%d", a+b);
								$$ = MakeNode(0); $$->type = $1->type; strcpy($$->lexeme,cons);
...
shift
	: plus_minus				{$$=$1;}
	| shift LEFT_OP plus_minus     {if($1->array == 200 && $3->array==200){ //means that both sides are constants
...
expression
	: conditional_exp						{$$=$1;}
	| unary_exp ass_operator expression 	{ int func(int status){
										char *exp = NULL;
										if (status) {
											exp = (char *)malloc(10);
											if ( exp ==NULL) {
